(() => {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        const categoriesItem = document.querySelectorAll(".menu__item")[1];
        const subMenuList = categoriesItem.querySelector(".sub-menu__list");
        categoriesItem.addEventListener("click", (function(event) {
            if (event.target.className !== "sub-menu__link") {
                event.preventDefault();
                subMenuList.classList.toggle("_open");
                categoriesItem.classList.toggle("_open");
            }
        }));
    }));
    const cardContainer = document.querySelector(".tools__items");
    const loadMoreButton = document.querySelector(".tools__button");
    const cards = cardContainer.children;
    const cardLimit = cardContainer.getAttribute("limit");
    ...
    const addCards = pageIndex => {
        ...
        for (let i = startRange; i <= endRange - 1; i++) cards[i].classList.add("_show");
    };
